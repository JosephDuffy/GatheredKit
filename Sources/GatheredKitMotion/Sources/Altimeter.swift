...
public final class Altimeter: UpdatingSource, Controllable, ActionProvider {
    ...
    @OptionalLengthProperty public private(set) var relativeAltitude: Measurement<UnitLength>?
    @OptionalPressureProperty public private(set) var pressure: Measurement<UnitPressure>?

    public var allProperties: [AnyProperty] { return [$relativeAltitude, $pressure] }

    public var actions: [Action] {
        return [
            Action(
                title: "Reset Altitude",
                isAvailable: isUpdating,
                perform: { [weak self] in guard let self = self else { return }
    ...
    public func startUpdating() {
        ...
        case .permissionDenied:
            sourceEventsSubject.notifyUpdateListeners(of: .failedToStart(error: .permissionDenied))
            return
        case .requiresPermissionsPrompt:
            // Perhaps it will ask the user when `startRelativeAltitudeUpdates` is called?
            break
        case .restricted:
            sourceEventsSubject.notifyUpdateListeners(of: .failedToStart(error: .restricted))
            return
        case .unavailable:
            sourceEventsSubject.notifyUpdateListeners(of: .failedToStart(error: .unavailable))
            return
        }
        ...
        altimeter.startRelativeAltitudeUpdates(to: updatesQueue) {
            [weak self, weak altimeter] data, error in guard let self = self else { return }
            if let error = error {
