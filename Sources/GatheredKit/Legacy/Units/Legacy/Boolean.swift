...
public struct Boolean: Unit {
    ...
    /**
     Create a new `Boolean` with `trueString` set to "true" and `falseString` set to "false"
     */
    public init() {
        self.init(trueString: "true", falseString: "false")
    }

    /**
     Create a new `Boolean` instance

     - parameter trueString: The string to return from `formattedString(for:)` when the value is `true`
     - parameter falseString: The string to return from `formattedString(for:)` when the value is `false`
     */
