...
open class BasePollingSource: UpdatingSource {
    ...
    open var availability: SourceAvailability { return .available }

    open var name: String { return "" }

    public final var updateInterval: TimeInterval? {
        switch state {
        case .monitoring(_, let updateInterval): return updateInterval
        case .notMonitoring: return nil
        }
    }
    ...
    private var updatesQueue: DispatchQueue? {
        switch state {
        case .monitoring(let updatesQueue, _): return updatesQueue
        case .notMonitoring: return nil
        }
    }
...
where Self: BasePollingSource, Self: ManuallyUpdatablePropertiesProvider {
    ...
    public func stopUpdating() { state = .notMonitoring }
