    ...
    override func spec() {
        describe("a screen") {
            ...
            context("that has not had `refreshData()` called") {
                it("will not have a value for the reported screen resolution") {
                    expect(screen.reportedScreenResolution.value).to(beNil())
                }

                it("will not have a value for the reported screen scale") {
                    expect(screen.reportedScreenScale.value).to(beNil())
                }

                it("will not have a value for the native screen resolution") {
                    expect(screen.nativeScreenResolution.value).to(beNil())
                }

                it("will not have a value for the native screen scale") {
                    expect(screen.nativeScreenScale.value).to(beNil())
                }

                it("will not have a value for the brightness") {
                    expect(screen.brightness.value).to(beNil())
                }
            }
